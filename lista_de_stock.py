...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.header = ["Código", "Fornecedor", "No do Documento", "Data", "Valor Documento", "Valor Pago", "Taxa",
                       "Subtotal", "Total", "Data de Criação", "Data de Modificação", "Modificado Por", "Criado Por", "Estado"]
    ...
    def fill_table(self):
        ...
        self.sql = """select * from stock where numero like "%{numero}%" or fornecedor like "%{fornecedor}%" 
    ...
    def update_data(self):
        ...
        if self.current_id == "":
            ...
            return
        sql = """ SELECT cod from stock where cod="{cod}" and estado="completo" """.format(cod=self.current_id)

        self.cur.execute(sql)
        data = self.cur.fetchall()

        if len(data) > 0:
            QMessageBox.information(self, "Documento completo", "Documento completo não pode ser modificado")
            return
